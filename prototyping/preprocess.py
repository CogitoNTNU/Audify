...
# CONFIGURATION
# =========================
DATASET_PATH = "./prototyping/dataset/audio/vol6_split"  # folder with only wav files
...
# PROCESS DATASET
# =========================
wav_files = glob(os.path.join(DATASET_PATH, "*.wav"))
...
for wav_file in tqdm(wav_files, desc="Processing WAV files"):
    ...
    np.save(os.path.join(OUTPUT_PATH, mel_name), mel)

    # Save waveform + mel image
    if SAVE_IMAGES:
        img_name = os.path.splitext(os.path.basename(wav_file))[0] + ".png"
        save_wave_and_mel_image(y, mel, os.path.join(OUTPUT_PATH, "images", img_name))
